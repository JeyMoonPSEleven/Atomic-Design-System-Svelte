...
const variants = [
		{
			title: 'Default',
			component: () => {},
			code: '<Settings logo={logo} navigation={navigation} sidebarItems={sidebar} children={content} footer={footer} />'
		}
	];
...
const props = [
		...
		},
		{
			...
			default: 'required',
			required: true,
			description: 'Contenido de configuración'
		},
		{
			name: 'footer',
			type: 'Snippet',
...
<div class="space-y-6">
	...
	<ComponentShowcase
		name="Settings"
		category="Templates"
		description="Plantilla de layout para páginas de configuración con sidebar, header y contenido principal."
		{preview}
		code={codeExample}
		{props}
		{examples}
