<script lang="ts">
	...
	{#snippet error404Variant()}
		<div class="border border-border-default rounded-lg overflow-hidden h-64">
			<Error code={404} title="No encontrado" message="La página que buscas no existe" />
		</div>
	{/snippet}

	{#snippet error500Variant()}
		<div class="border border-border-default rounded-lg overflow-hidden h-64">
			<Error code={500} title="Error del servidor" message="Algo salió mal en el servidor" />
		</div>
	{/snippet}

	{#snippet withActionVariant()}
		<div class="border border-border-default rounded-lg overflow-hidden h-64">
			<Error code={404} title="No encontrado" action={{ label: 'Volver al inicio', href: '/' }} />
		</div>
	{/snippet}
	...
	{#snippet examples()}
		<div class="space-y-6">
			<div>
				<h4 class="font-semibold mb-3 text-text-default">Página de Error</h4>
